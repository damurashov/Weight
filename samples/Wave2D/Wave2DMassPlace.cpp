...
void Wave2DMassPlace::init() {
	...
	currentWave = 0;
	outMessage = (void*) new double;
	*(double*)outMessage = previousWave;
...
void Wave2DMassPlace::computeFirstTurn() {
	...
	currentWave = previousWave + 
		c * c / 2.0 * dt * dt / (dd * dd) * 
		(getOtherWaveState(north) + getOtherWaveState(east) + getOtherWaveState(south) + 
		getOtherWaveState(west) - 4.0 * previousWave);
...
void Wave2DMassPlace::computeWave() {
	...
	double nextWave = 2.0 * currentWave - previousWave +
		c * c * dt * dt / (dd * dd) *
		(getOtherWaveState(north) + getOtherWaveState(east) + getOtherWaveState(south) +
		getOtherWaveState(west) - 4.0 * currentWave);
...
double Wave2DMassPlace::getOtherWaveState(int* relativeIndex) {
	...
	if (myX + relativeIndex[0] >= sizeX || myX + relativeIndex[0] < 0 ||
		...
		otherWave = *(double*) getOutMessage(1, relativeIndex);
	}
	return otherWave;
}
