  ...
  while ( running ) {
    ...
    switch ( status ) {
      ...
      case STATUS_CALLALL:
        ...
        if ( printOutput == true ) {
          convert.str( "" );
          convert << "Mthread[" << tid << "] works on CALLALL:" << " placese = "
              << (void *) places << " functionId = " << functionId
              << " argument = " << argument << " arg_size = " << arg_size
              << " msgType = " << msgType << " ret_size = " << ret_size;
          MASS_base::log( convert.str( ) );
        }
      ...
      case STATUS_AGENTSCALLALL:
        ...
        if ( printOutput == true ) {
          ...
          convert << "Mthread[" << tid << "] works on AGENST_CALLALL:"
              << " agents = " << (void*) agents << " functionId = "
              << functionId << " argument = " << argument << " arg_size = "
              << arg_size << " msgType = " << msgType << " ret_size = "
              << ret_size;
          MASS_base::log( convert.str( ) );
        }
