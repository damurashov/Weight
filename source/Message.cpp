 ...
 *   <li>RemoteExchangeRequest Objects held in exchangeReqList (does not delete list)</li>
 *   <li>AgentMigrationRequest Objects held in migrationReqList (does not delete list)</li>
 * </ol>
 */
Message::~Message( ) {
  if ( argument_in_heap == true && argument != NULL )
    delete (char *) argument;
  if ( hosts != NULL )
    delete hosts;
  if ( argument_in_heap == true && destinations != NULL )
    delete destinations;
...
char *Message::serialize( int &msg_size ) {
  ...
  switch ( action ) {
    ...
    case PLACES_CALL_SOME_RETURN_OBJECT:
      // calculate msg_size
      msg_size += sizeof(int);    // int handle,
      msg_size += sizeof(int);    // int functionId
      msg_size += sizeof(int);    // argument_size;
    ...
    case PLACES_EXCHANGE_ALL_REMOTE_REQUEST:
      ...
      *(int *) pos = exchangeReqList->size( );
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert << " functionId = " << functionId << " exchangeReqList->size = "
    ...
    case AGENTS_MIGRATION_REMOTE_REQUEST:
      ...
      *(int *) pos = migrationReqList->size( );
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert << "Agent handle: " << handle << " place handle: "
            << dest_handle << " size = " << migrationReqList->size( );
        MASS_base::log( convert.str( ) );
      }

      // process each migrationReq
      for ( int i = 0; i < int( migrationReqList->size( ) ); i++ ) {
...
void Message::deserialize( char *msg, int msg_size ) {
  ...
  switch ( *(ACTION_TYPE *) cur ) {
    ...
    case PLACES_EXCHANGE_ALL_REMOTE_REQUEST:
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert << "PLACES_EXCHANGE_ALL_REMOTE_REQUEST: " << " action = "
            << action << " handle = " << handle << " dest_handle = "
            << dest_handle << " functionId = " << functionId
      ...
      for ( int i = 0; i < exchangeReqListSize; i++ ) {
        ...
        if ( printOutput == true ) {
          convert.str( "" );
          convert << "PLACES_EXCHANGE_ALL_REMOTE_REQUEST:" << " i = " << i
              << " destIndex = " << destIndex << " orgIndex = " << orgIndex
              << " inMsgIndex = " << inMsgIndex << " inMsgSize = " << inMsgSize
              << " outMessageSize = " << outMessageSize << " outMessage = "
              << *(int *) outMessage;
          MASS_base::log( convert.str( ) );
        }

        RemoteExchangeRequest *request = new RemoteExchangeRequest( destIndex,
            orgIndex, inMsgIndex, inMsgSize, outMessage, outMessageSize, true );
    ...
    case AGENTS_MIGRATION_REMOTE_REQUEST:
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert << "Deserialize: Agent handle: " << handle << " place handle: "
            << dest_handle;
        MASS_base::log( convert.str( ) );
      }
      ...
      for ( int i = 0; i < migrationReqListSize; i++ ) { // each migrationReq
        ...
        if ( printOutput == true ) {
          convert.str( "" );
          convert << "Deserialize: agentId(" << agent << ")[" << agent->agentId
        ...
        if ( agent->migratableDataSize > 0 ) {
          ...
          if ( printOutput == true ) {
            ...
            convert << "Deserialize: agentId(" << agent << ")["
                << agent->agentId << "] malloc = " << agent->migratableData;
            MASS_base::log( convert.str( ) );
          }
    ...
    case PLACES_EXCHANGE_BOUNDARY_REMOTE_REQUEST:
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert
            << "PLACES_EXCHANGE_ALL_REMOTE_RETURN_OBJECT: deserialization done"
            << " argument_size = " << argument_size;
        MASS_base::log( convert.str( ) );
      }
    ...
    case AGENTS_INITIALIZE:
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert << "population = " << agent_population << " handle = " << handle
            << " placeHandle = " << dest_handle << " classname = " << classname
            << " argument_size = " << argument_size;
        MASS_base::log( convert.str( ) );
      }
