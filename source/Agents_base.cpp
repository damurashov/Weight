...
Agents_base::Agents_base( int handle, string className, void *argument,
    ...
    handle( handle ), className( className ), placesHandle( placesHandle ), initPopulation(
        initPopulation ) {
  ...
  if ( printOutput == true ) {
    convert << "handle = " << handle << ",placesHandle = " << placesHandle
        << ", class = " << className << ", argument_size = " << argument_size
        << ", argument = " << (char *) argument << ", initPopulation = "
        << initPopulation << endl;
  ...
  if ( printOutput == true ) {
    ...
    convert << "Agets_base constructor: placesDllClass = "
        << (void *) placesDllClass << " curPlaces = " << (void *) curPlaces;
    MASS_base::log( convert.str( ) );
  }
  for ( int i = 0; i < curPlaces->getPlacesSize( ); i++ ) {
    ...
    for ( int nColonists = protoAgent->map( initPopulation, curPlace->size,
        curPlace->index, curPlace ); nColonists > 0;
        nColonists--, localPopulation++ ) {
    ...
    if ( myIndex > 0 ) {
      ...
      if ( printOutput == true ) {
        ...
        convert << "Thread[" << tid << "]: agent(" << myIndex
            << "): MASS_base::currentReturns  = " << MASS_base::currentReturns
            << " ret_size = " << ret_size;
        MASS_base::log( convert.str( ) );
      }

      char *return_values = MASS_base::currentReturns
          + ( myIndex - 1 ) * ret_size;
...
void Agents_base::getGlobalAgentArrayIndex( vector<int> src_index,
    int dst_size[], int dest_dimension, int dest_index[] ) {
...
void Agents_base::manageAll( int tid ) {
  ...
  while ( true ) {
    ...
    if ( printOutput == true ) {
      convert.str( "" );
      convert << "agent " << evaluationAgent->agentId << "'s childrenCounter = "
          << childrenCounter;
      MASS_base::log( convert.str( ) );
    }
    ...
    while ( childrenCounter > 0 ) {
      if ( printOutput == true ) {
        ...
        convert << "Agent_base::manageALL: Thread " << tid
            ...
            << "...arguments.size( ) = " << evaluationAgent->arguments.size( )
    ...
    if ( printOutput == true ) {
      convert.str( "" );
      convert << "Agent_base::manageALL: Thread " << tid << " check "
          << evaluationAgent->agentId << "'s alive = "
    ...
    if ( evaluationAgent->alive == false ) {
      ...
      for ( int i = 0; i < evalPlaceAgents; i++ ) {
        ...
        if ( ( evaluationAgent->agentId == convertedAgent->agentId )
          ...
          if ( printOutput == true ) {
            convert.str( "" );
            convert << "Agent_base::manageALL: Thread " << tid << " deleted "
                << evaluationAgent->agentId << " from place["
    ...
    if ( printOutput == true ) {
      ...
      convert << "pthread_self[" << pthread_self( ) << "tid[" << tid
          << "]: calls from" << "[" << evaluationAgent->index[0] << "]["
          << evaluationAgent->index[1] << "]" << " (destCoord[" << destCoord[0]
          << "][" << destCoord[1] << "]" << " evaluatedPlaces->size["
          << evaluatedPlaces->size[0] << "][" << evaluatedPlaces->size[1]
          << "]";
      MASS_base::log( convert.str( ) );
    }
    ...
    if ( destCoord[0] != -1 ) {
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert << "getGlobalLinearIndexFromGlobalArrayIndex: evaluatedPlace = "
      ...
      int globalLinearIndex =
          evaluatedPlaces->getGlobalLinearIndexFromGlobalArrayIndex( destCoord,
              evaluatedPlaces->size, evaluatedPlaces->dimension );
      if ( printOutput == true ) {
        convert.str( "" );
        convert << " linear = " << globalLinearIndex << " lower = "
            << evaluatedPlaces->lower_boundary << " upper = "
      ...
      if ( globalLinearIndex >= evaluatedPlaces->lower_boundary
        ...
        if ( printOutput == true ) {
          convert.str( "" );
          convert << "evaluationAgent->place = " << evaluationAgent->place;
          MASS_base::log( convert.str( ) );
        }
        ...
        // create a request
        AgentMigrationRequest *request = new AgentMigrationRequest(
            globalLinearIndex, evaluationAgent );
  ...
  if ( tid == 0 ) {
    ...
    for ( int rank = 0; rank < MASS_base::systemSize; rank++ ) {
      ...
      if ( pthread_create( &thread_ref[rank], NULL,
          Agents_base::processAgentMigrationRequest, comThrArgs[rank] ) != 0 ) {
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert
            << "Agents_base.manageAll will start processAgentMigrationRequest thread["
    ...
    for ( int rank = MASS_base::systemSize - 1; rank >= 0; rank-- ) {
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert
            << "Agents_base.manageAll will join processAgentMigrationRequest A thread["
            << rank << "] = " << thread_ref[rank] << " myPid = "
            << MASS_base::myPid;
        MASS_base::log( convert.str( ) );
      }
      if ( rank == MASS_base::myPid ) // don't communicate with myself
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert
            << "Agents_base.manageAll will join processAgentMigrationRequest B thread["
            << rank << "] = " << thread_ref[rank];
        MASS_base::log( convert.str( ) );
      }
      ...
      if ( printOutput == true ) {
        convert.str( "" );
        convert
            << "Agents_base.manageAll joined processAgentMigrationRequest C thread["
            << rank << "] = " << thread_ref[rank];
        MASS_base::log( convert.str( ) );
      }
    }
...
void *Agents_base::sendMessageByChild( void *param ) {
  ...
  ostringstream convert;
  if ( printOutput == true ) {
    convert << "pthread_self[" << pthread_self( ) << "] sendMessageByChild to "
        << rank << " starts";
    MASS_base::log( convert.str( ) );
  }
