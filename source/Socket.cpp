...
Socket::~Socket() {
  ...
  convert << "~Socket called: clientFd = " << clientFd << " serverFd = "
      << serverFd;
...
int Socket::getClientSocket(const char ipName[]) {
  ...
  sendSockAddr.sin_addr.s_addr = inet_addr(
  ...
  const int on = 1;
  if (setsockopt(clientFd, IPPROTO_TCP, TCP_NODELAY, (char *) &on, sizeof(int))
      < 0) {
  ...
  while (connect(clientFd, (sockaddr*) &sendSockAddr, sizeof(sendSockAddr)) < 0)
    ;
...
int Socket::getServerSocket() {
  if (serverFd == NULL_FD) { // Server not ready
    ...
    if (bind(serverFd, (sockaddr*) &acceptSockAddr, sizeof(acceptSockAddr))
        < 0) {
      perror(
          "Cannot bind the local address to the server socket. bind() call returned");

      cerr << "serverFd: " << serverFd << endl;
      cerr << "acceptSockAddr.sin_family: " << acceptSockAddr.sin_family
          << endl;
      cerr << "acceptSockAddr.sin_addr.s_addr: "
          << acceptSockAddr.sin_addr.s_addr << endl;
      cerr << "acceptSockAddr.sin_port: " << acceptSockAddr.sin_port << endl;
  ...
  if ((newFd = accept(serverFd, (sockaddr*) &newSockAddr, &newSockAddrSize))
      < 0) {
