...
Agents::Agents( int handle, string className, void *argument, int argument_size,
    ...
    Agents_base( handle, className, argument, argument_size,
        places->getHandle( ), initPopulation ), localAgents(
        new int[MASS_base::systemSize] ) {
...
void Agents::init_master( void *argument, int argument_size ) {
  ...
  for ( int i = 0; i < MASS_base::systemSize; i++ ) {
    ...
    if ( printOutput == true ) {
      cerr << "rank[" << i << "]'s local agent population = " << localAgents[i]
          << endl;
    }
  }
...
void *Agents::ca_setup( int functionId, void *argument, int arg_size,
  ...
  for ( int i = 0; i < int( MASS::mNodes.size( ) ); i++ ) {
    ...
    if ( type == Message::AGENTS_CALL_ALL_VOID_OBJECT )
      ...
      for ( int dest = 0; dest <= i; dest++ ) {
        ...
        if ( printOutput == true ) {
          cerr << "Agents.callAll: calc arg_pos = " << arg_pos
              << " localAgents[" << dest << "] = " << localAgents[dest] << endl;
        }
      }
      ...
      if ( printOutput == true ) {
        cerr << "Agents.callAll: to rank[" << ( i + 1 ) << "] arg_size = "
            << arg_size << " arg_pos = " << arg_size * arg_pos << " arg_size = "
            << arg_size * localAgents[i + 1] << endl;
      }
    }
  ...
  MASS_base::currentReturns = new char[total * MASS_base::currentRetSize]; // prepare an entire return space
  // resume threads
  ostringstream convert;
