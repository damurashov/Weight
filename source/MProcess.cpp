      ...
      switch ( m->getAction( ) ) {
        ...
        case Message::PLACES_INITIALIZE:
          ...
          places = new Places_base( m->getHandle( ), m->getClassname( ),
              m->getBoundaryWidth( ), argument, argument_size, size.size( ),
              &size[0] );
          ...
          MASS_base::placesMap.insert(
              map<int, Places_base*>::value_type( m->getHandle( ), places ) );
          sendAck( );
          if ( printOutput == true )
        ...
        case Message::PLACES_CALL_ALL_RETURN_OBJECT:
          ...
          if ( printOutput == true ) {
            ostringstream convert;
            convert << "check 2 currentArgSize = "
          ...
          // confirm all threads are done with places.callAll and return objects
          Mthread::barrierThreads( 0 );
        ...
        case Message::PLACES_EXCHANGE_ALL:
          ...
          MASS_base::currentPlaces->exchangeAll( MASS_base::destinationPlaces,
              MASS_base::currentFunctionId, MASS_base::currentDestinations, 0 );
        ...
        case Message::AGENTS_INITIALIZE:
          ...
          agents = new Agents_base( m->getHandle( ), m->getClassname( ),
              argument, argument_size, m->getDestHandle( ),
              m->getAgentPopulation( ) );

          MASS_base::agentsMap.insert(
              map<int, Agents_base*>::value_type( m->getHandle( ), agents ) );
